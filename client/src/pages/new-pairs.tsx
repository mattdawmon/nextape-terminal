...
export default function NewPairsPage() {
  ...
  const newTokens = useMemo(
    () => tokens.filter((t) => t.isNew).sort((a, b) => {
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-border">
          ...
          <span className="font-bold text-sm">{t.newPairs.title}</span>
          <span className="text-xs text-muted-foreground font-mono">({newTokens.length})</span>
        </div>
