...
export default function TrendingPage() {
  ...
  const trendingTokens = useMemo(
    () => tokens.filter((t) => t.isTrending).sort((a, b) => (b.volume24h ?? 0) - (a.volume24h ?? 0)),
    [tokens]
  );
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-border">
          ...
          <span className="font-bold text-sm">{t.trending.title}</span>
          <span className="text-xs text-muted-foreground font-mono">({trendingTokens.length})</span>
        </div>
